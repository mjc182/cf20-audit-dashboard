...
.block-container { padding-top: 1.25rem; padding-left: 1.4rem; padding-right: 1.4rem; max-width: 1500px; }
#MainMenu, header, footer { visibility: hidden; }
.page-title { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:18px; }
.title-left h1 { margin:0; font-size:1.95rem; line-height:1.1; letter-spacing:-0.05em; color:#f8fafc; }
.title-left p { margin:8px 0 0 0; color:#cbd5e1; font-size:1.02rem; }
.fake-controls { display:flex; gap:12px; align-items:center; }
.fake-select { border:1px solid rgba(148,163,184,0.22); color:#e2e8f0; background:#091827; border-radius:9px; padding:9px 14px; font-weight:600; min-width:105px; text-align:center; }
.metric-card { position:relative; border:1px solid rgba(148,163,184,0.18); background:linear-gradient(145deg, rgba(15,23,42,.95), rgba(15,32,55,.88)); border-radius:12px; padding:19px 20px; min-height:130px; box-shadow:inset 0 1px 0 rgba(255,255,255,0.03), 0 18px 34px rgba(0,0,0,0.22); overflow:hidden; }
.metric-card .label { color:#cbd5e1; font-size:.92rem; font-weight:600; }
.metric-card .value { margin-top:14px; font-size:1.75rem; font-weight:800; letter-spacing:-0.04em; }
.metric-card .sub { margin-top:8px; font-size:.87rem; color:#22c55e; font-weight:700; }
.metric-icon { position:absolute; right:20px; top:31px; width:66px; height:66px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:2rem; opacity:.95; }
...
.blue { color:#3b82f6; } .green { color:#22c55e; } .yellow { color:#fbbf24; } .red { color:#ef4444; }
.panel { border:1px solid rgba(148,163,184,0.18); background:linear-gradient(145deg, rgba(15,23,42,.92), rgba(10,25,44,.86)); border-radius:12px; padding:16px 17px; box-shadow:0 18px 34px rgba(0,0,0,.20); }
.panel-title { font-size:1rem; font-weight:800; color:#f8fafc; margin-bottom:10px; }
.mini-link { color:#38bdf8; font-size:.88rem; margin-top:10px; }
.statusbar { margin-top:16px; border-top:1px solid rgba(148,163,184,.13); padding:13px 0 3px 0; display:flex; justify-content:space-between; color:#cbd5e1; font-size:.86rem; }
.dot { display:inline-block; width:9px; height:9px; border-radius:50%; background:#22c55e; margin-right:7px; }
.sidebar-brand { font-size:1.45rem; font-weight:800; color:#f8fafc; padding:12px 0 18px 0; }
.sidebar-pill { background:#2563eb; border-radius:9px; padding:11px 14px; color:white; font-weight:700; margin-bottom:8px; }
.sidebar-item { padding:10px 14px; color:#cbd5e1; font-weight:500; margin-bottom:4px; }
small { color:#94a3b8; }
...
def fmt_tokens(raw, decimals=2):
    try: return f"{float(raw) / SCALE:,.{decimals}f}"
    except Exception: return "0.00"

def fmt_compact_tokens(raw):
    try: n = float(raw) / SCALE
...
def fetch_zerochain_mints(pages=40, limit=40):
    ...
    for page in range(pages):
        offset = page * limit
        try:
            r = requests.get(ZEROCHAIN_API, params={"limit": limit, "offset": offset, "reverse": "true"}, timeout=20)
            r.raise_for_status()
            payload = r.json()
        except Exception:
            break
        for item in payload.get("blocks_or_event", []):
            for datum in item.get("datums", []):
                if datum.get("type") == "DATUM_TOKEN_EMISSION":
                    d = datum.get("data", {})
                    validators = d.get("valid_sign_hashes", []) or [s.get("pkey_hash") for s in d.get("data", []) if s.get("pkey_hash")]
                    row = {
                    ...
                    rows.append(row)
                    raw_events.append({"from": "MINT_AUTHORITY", "to": row["to"], "amount": row["amount"], "token": row["token"], "time": row["time"], "label_from": "Validator", "label_to": "Bridge" if row["validator_count"] >= 3 else "User"})
...
def safe_burn_total(rpc, token, start_block):
    ...
    try:
        ...
        latest, step, total = w3.eth.block_number, 50_000, 0
        target_topic = Web3.to_hex(Web3.to_bytes(hexstr=BURN).rjust(32, b"\x00"))
        for block in range(start_block, latest + 1, step):
            logs = w3.eth.get_logs({"fromBlock": block, "toBlock": min(block + step - 1, latest), "address": Web3.to_checksum_address(token), "topics": [TRANSFER_TOPIC, None, target_topic]})
            for log in logs:
...
def label_wallets(df):
    ...
    def label(row):
        if row["share"] > 0.20 and row["txs"] > 5: return "Bridge"
        if row["share"] > 0.08: return "Whale"
        if row["txs"] > 25: return "Exchange"
        return "User"
...
def build_bridge_wallet_detector(wallet_df, events):
    ...
    candidates = scored[scored["bridge_score"] >= 60].sort_values("bridge_score", ascending=False)
...
<div class="page-title">
  ...
  <div class="fake-controls"><div class="fake-select">All Tokens⌄</div><div class="fake-select">7D⌄</div></div>
</div>
...
wallet_df, val_df = label_wallets(df), validator_stats(df)
...
total_locked, diff = eth_burned + bsc_burned, total_minted - (eth_burned + bsc_burned)
...
backing_score = proof_of_backing_score(diff, total_minted, bridge_candidates_df, val_df, bridge_movements)
...
with right:
    ...
    if wallet_df.empty:
        ...
        st.bar_chart(dist.set_index("label")["tokens"], height=245)
        for _, row in dist.sort_values("amount", ascending=False).iterrows():
            st.markdown(f"<small>■ <b>{row['label']} Wallets</b> — {row['tokens']:,.2f} ({row['share']:.1%})</small>", unsafe_allow_html=True)
...
with b1:
    ...
    if df.empty:
        ...
        recent = recent.rename(columns={"time": "Time (UTC)", "token": "Token", "validator_count": "Type"})
        st.dataframe(recent[["Time (UTC)", "Token", "Amount", "To", "Type"]], use_container_width=True, hide_index=True)
...
with b2:
    ...
    if wallet_df.empty:
        ...
        st.dataframe(top[["Address", "Label", "Balance (CF20)", "% of Supply"]], use_container_width=True, hide_index=True)
...
with b3:
    ...
    if bridge_candidates_df.empty:
        ...
        transfers["Type"] = "↓"
        st.dataframe(transfers[["Time (UTC)", "From", "To", "Amount (CF20)", "Type"]], use_container_width=True, hide_index=True)
