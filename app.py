...
if missing_cell is not None:
    missing_mcell_equiv = float(missing_cell) / CELL_PER_MCELL
    missing_mcell_display = fmt_num(missing_mcell_equiv)
else:
    missing_mcell_display = "15.8K–16.0K"
...
st.info(
    "Unit note: throughout this dashboard, 1 mCELL = 1,000 CELL. "
...
<div class="panel">
  ...
  <div style="color:#cbd5e1;line-height:1.65;">
    Independent emission matching identified approximately
    <b>{missing_display} CELL</b>, equal to approximately
    <b>{missing_mcell_display} mCELL-equivalent</b>, that could not be matched
    to known ETH/BSC bridge deposit flows.

    <br><br>

    The official disclosure refers to <b>{OFFICIAL_ILLEGAL_MCELL:,} illegal mCELL</b>,
    equivalent to approximately <b>{OFFICIAL_CELL_EQUIV:,.0f} CELL</b>
    at <b>1 mCELL = 1,000 CELL</b>.

    <br><br>

    This means the independent unmatched-emission estimate is materially larger
    than the official 1,295 mCELL figure unless explained by unit interpretation,
    duplicate emissions, non-mCELL CELL emissions, or additional matching data.

    <br><br>

    A separate <b>DATUM_TX</b> record links the largest unmatched recipient wallet
    to a <b>BRIDGE OUT BEP20</b> transaction. The amount definitively sold on
    open markets remains unresolved pending BSC/DEX/OTC destination tracing.
  </div>
</div>
""",
    unsafe_allow_html=True,
...
with c3:
    metric(
        "Official illegal mCELL",
        f"{OFFICIAL_ILLEGAL_MCELL:,}",
        "Cellframe statement",
        "warn",
    )
...
with c4:
    metric(
        "Official CELL-equivalent",
        fmt_num(OFFICIAL_CELL_EQUIV),
        "1,295 × 1,000",
        "purple",
    )
        ...
        [
            {
                ...
                "Status": "Proven by current scripts",
                "Evidence": "≈15.8M–16.0M CELL, equal to ≈15,800–16,000 mCELL-equivalent",
            },
            {
                "Finding": "Official illegal mCELL disclosure",
                "Status": "Disclosed by Cellframe statement",
                "Evidence": "1,295 mCELL × 1,000 CELL = 1,295,000 CELL-equivalent",
            },
            {
...
with left:
    ...
    if wallets.empty:
        st.info(
            "missing_cell_wallets.csv not found. "
            "Run filter_missing_cell_only.py and upload the output."
        )
    else:
        show = wallets.head(10).copy()

        if "missing_cell" in show.columns:
            show["missing_mcell_equivalent"] = (
                pd.to_numeric(show["missing_cell"], errors="coerce") / CELL_PER_MCELL
            )

            show["missing_cell"] = pd.to_numeric(
                show["missing_cell"],
                errors="coerce",
            ).map(lambda x: f"{x:,.2f}")

            show["missing_mcell_equivalent"] = show[
...
if not wallets.empty and {"mint_to", "missing_cell"}.issubset(wallets.columns):
    ...
    chart_df["missing_cell"] = pd.to_numeric(
        chart_df["missing_cell"],
        errors="coerce",
    )
    ...
    chart = (
        ...
        .encode(
            ...
            tooltip=[
                ...
                alt.Tooltip(
                    "missing_mcell_equivalent:Q",
                    title="mCELL Equivalent",
                    format=",.2f",
                ),
            ],
        )
