...
# Current verified Gate.io-bound route exposure from traced BSC supply paths.
gateio_verified = 2596567.55466338
...
with cols[3]:
    kpi_card("Verified Gate.io route", f"{compact(gateio_verified)}", "2,596,567.55 CELL verified traced BSC mint-path exposure to Gate.io-labelled endpoint", "▣", "green")
with cols[4]:
...
# ==============================
# CENTRAL BSC SUPPLY WALLET TRACE SUMMARY
# ==============================

trace_65def = load_json(Path("auto_trace_65def_summary.json"))
trace_c3b8 = load_json(Path("auto_trace_c3b8_summary.json"))
trace_da8a = load_json(Path("auto_trace_da8a_summary.json"))

st.markdown('<a id="central-wallet-traces"></a>', unsafe_allow_html=True)
...
<div class="section">
  <div class="section-title">
    <div>
      <h2>Central BSC Supply Wallets: Completed Traces</h2>
      <p>The two core BSC supply wallets have now been traced to zero balance. These traces explain how the BSC-minted supply moved into downstream custody, distribution, and Gate.io-labelled routes.</p>
    </div>
    <div class="badge">Central traces complete</div>
  </div>

  <div class="kpi-grid" style="grid-template-columns:repeat(4,1fr);">
    <div class="kpi-card tone-green">
      <div class="kpi-label">0x65def mint recipient</div>
      <div class="kpi-value">0 CELL</div>
      <div class="kpi-sub">Original BSC mint recipient traced to zero balance</div>
    </div>
    <div class="kpi-card tone-blue">
      <div class="kpi-label">0xc3b8 primary custody</div>
      <div class="kpi-value">0 CELL</div>
      <div class="kpi-sub">Primary 30M custody wallet traced to zero balance</div>
    </div>
    <div class="kpi-card tone-purple">
      <div class="kpi-label">0xda8a consolidation</div>
      <div class="kpi-value">0 CELL</div>
      <div class="kpi-sub">Consolidation wallet traced to Gate.io-labelled endpoint</div>
    </div>
    <div class="kpi-card tone-orange">
      <div class="kpi-label">Verified Gate.io-bound route</div>
      <div class="kpi-value">2.60M</div>
      <div class="kpi-sub">At least 2,596,567.55 CELL routed into Gate.io-labelled custody</div>
    </div>
  </div>
  ...
  <div class="success-note">
    <b>New audit finding:</b> The original BSC mint recipient <code>0x65def...</code> and the primary BSC custody wallet <code>0xc3b8...</code> have both been traced to zero balance. Verified Gate.io-bound route exposure from traced BSC supply paths is now at least <b>2,596,567.55 CELL</b>.
  </div>

  <br>

  <div class="note">
    <b>Important caveat:</b> This proves routing into a Gate.io-labelled custody endpoint. It does not prove exact realized sale proceeds, sale price, exchange-internal trading, or final beneficiaries after deposit.
  </div>
</div>
    """,
    unsafe_allow_html=True,
...
central_rows = [
    {
        ...
        "Key result": "39.31M outbound, including 30M to 0xc3b8, 4.0001M to 0x8bbf, 2.635M to 0xda8a, and 656,567.55 CELL direct to Gate.io",
    },
    {
        "Wallet": "0xc3b8a652e59d59a71b00808c1fb2432857080ab8",
        "Role": "Primary BSC custody wallet",
        "Trace status": "Traced to zero",
        "Key result": "31.10M outbound across 19 recipients, including 6.4M back to 0x65def and 1M to 0xda8a",
    },
    {
        ...
        "Key result": "940,000 CELL routed to Gate.io-labelled endpoint in the traced segment",
    },
    {
        "Wallet": "0x0d0707963952f2fba59dd06f2b425ace40b492fe",
        "Role": "Gate.io-labelled endpoint",
        "Trace status": "CEX custody endpoint",
        "Key result": "At least 2,596,567.55 CELL verified routed into this labelled endpoint from traced BSC supply paths",
    },
]

st.dataframe(pd.DataFrame(central_rows), use_container_width=True, hide_index=True)



# ==============================
# 8BBF DOWNSTREAM DISTRIBUTION TRACE
# ==============================

trace_8bbf = load_json(Path("auto_trace_8bbf_combined_summary.json"))
recipients_8bbf = load_csv(Path("auto_trace_8bbf_combined_recipient_summary.csv"))

st.markdown('<a id="trace-8bbf"></a>', unsafe_allow_html=True)
...
<div class="section">
  <div class="section-title">
    <div>
      <h2>Completed 8bbf Downstream Distribution Trace</h2>
      <p>The 0x8bbf branch was one of the largest unresolved branches from the BSC supply path. It has now been traced across multiple parts to zero balance.</p>
    </div>
    <div class="badge">Trace complete</div>
  </div>
  ...
  <div class="kpi-grid" style="grid-template-columns:repeat(5,1fr);">
    <div class="kpi-card tone-blue">
      <div class="kpi-label">Target wallet</div>
      <div class="kpi-value" style="font-size:1.05rem;">0x8bbf...e218</div>
      <div class="kpi-sub">Large downstream branch wallet</div>
    </div>
    <div class="kpi-card tone-green">
      <div class="kpi-label">Final balance</div>
      <div class="kpi-value">{compact(trace_8bbf.get("final_balance_cell", 0))} CELL</div>
      <div class="kpi-sub">Trace reached zero balance</div>
    </div>
    <div class="kpi-card tone-orange">
      <div class="kpi-label">Total outbound</div>
      <div class="kpi-value">{compact(trace_8bbf.get("total_outbound_cell", 0))}</div>
      <div class="kpi-sub">CELL distributed during completed trace</div>
    </div>
    <div class="kpi-card tone-purple">
      <div class="kpi-label">Unique recipients</div>
      <div class="kpi-value">{trace_8bbf.get("unique_recipients", "—")}</div>
      <div class="kpi-sub">Broad downstream distribution pattern</div>
    </div>
    <div class="kpi-card tone-blue">
      <div class="kpi-label">Outbound events</div>
      <div class="kpi-value">{trace_8bbf.get("outbound_events", "—")}</div>
      <div class="kpi-sub">Transfer events in combined trace</div>
    </div>
  </div>
  ...
  <div class="success-note">
    <b>Audit finding:</b> The <code>0x8bbf...</code> branch was traced to zero and behaved as a high-volume downstream distribution wallet, not a reserve-style wallet.
    Its largest recipient was <code>0x35ce...</code>, a known bridge/aggregator address, receiving <b>552,542.49 CELL</b>.
  </div>

  <br>

  <div class="note">
    <b>Interpretation:</b> This strengthens the downstream distribution model. The 8bbf branch did not retain reserve-like backing; it distributed funds across a broad set of recipients.
  </div>
</div>
        """,
        unsafe_allow_html=True,
    ...
    if not recipients_8bbf.empty:
        show = recipients_8bbf.copy()
...
<div class="section">
  <div class="section-title">
    <div>
      <h2>8bbf Downstream Distribution Trace</h2>
      <p>Run the 8bbf auto-trace and merge script to generate <code>auto_trace_8bbf_combined_summary.json</code>.</p>
    </div>
    <div class="badge orange">Pending trace output</div>
  </div>
</div>
