...
.bridge-image-section {
    ...
    background:
        ...
        radial-gradient(circle at 82% 12%, rgba(168,85,247,.10), transparent 24%),
...
def render_current_bridge_model_image():
    st.markdown(
        """
        <div class="bridge-image-section">
            <div class="bridge-image-top">
                <div>
                    <div class="bridge-image-heading">Current Bridge Model</div>
                    <div class="bridge-image-subheading">
                        Observed bridge infrastructure and downstream routing paths
                    </div>
                </div>
                <div class="bridge-image-chip">● Audit evidence: On-chain verified</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    if CURRENT_BRIDGE_IMAGE.exists():
        ...
        st.markdown(
            """
            <div class="bridge-image-note">
                This infographic summarizes the currently observed bridge-facing wallets,
                lock/unlock infrastructure, intake and aggregation layer, consolidation routes,
                and downstream market endpoints. Route exposure is not total supply and is not
                exact realized sale volume.
            </div>
            """,
            unsafe_allow_html=True,
        )
